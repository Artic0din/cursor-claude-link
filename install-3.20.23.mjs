...
if(version!=='3.20.23'||product.commit!=='b23e0e2d3c0fc9bb9311f4390230a120ccc9aa50')throw new Error('This patch supports Cursor 3.20.23 build b23e0e2 only.');
...
for(const surface of ['desktop','glass']){
  ...
  const models=desktop?'c':'l', mapper=desktop?'V=>tpn(V)':'U=>Azn(U)';
  ...
  if(source.includes('const __chatgptLocal=__isChatgptBridgeModel(u?.requestedModel?.modelId??i?.modelId);')){
    ...
    const before=desktop?'async run(e,t,n,i,r,s,o,a,c,l,u){const h=_jf(u,':'async run(t,e,n,i,r,s,o,a,l,c,u){const d=FyS(u,';
  ...
  const activation=desktop?'function kup(e){return Zc.localMode&&e?.get(gJ_,-1)==="true"}':'function SIg(t){return kl.localMode&&t?.get(AIg,-1)==="true"}';
  ...
  const usage=desktop
    ?{jsx:'zvy',useState:'Wlr',useEffect:'tvy',card:'b_',zs:'zs',bar:'xA',barStyle:'Klr',
      fn:'function Qvy(e){const t=hfp(119)',
      children:'title:"Plan & Usage",children:[yn,an,Kt,xn]',
      childrenGpt:'title:"Plan & Usage",children:[yn,an,Kt,xn,zvy(__chatgptUsageSection,{})]'}
    :{jsx:'hH1',useState:'dus',useEffect:'dH1',card:'nv',zs:'Js',bar:'Im',barStyle:'kTi',
      fn:'function gH1(t){const e=pDg(119)',
      children:'title:"Plan & Usage",children:[ft,wt,gt,Tt]',
      childrenGpt:'title:"Plan & Usage",children:[ft,wt,gt,Tt,hH1(__chatgptUsageSection,{})]'};
  ...
  source=patchSubagentBubbles(source,surface,'3.20.23');
  source=patchSubagentLifecycle(source,surface,'claude-subscription/','3.20.23');
...
if(process.argv.includes('--check')){console.log('Cursor 3.20.23 Claude patch candidates passed syntax and anchor checks.');process.exit();}
