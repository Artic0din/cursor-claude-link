...
if(version!=='3.21.9'||product.commit!=='9998796a6096ce83d83a9332bfe7473b985db750')throw new Error('This patch supports Cursor 3.21.9 build 9998796 only.');
...
for(const surface of ['desktop','glass']){
  ...
  source=patchPickerSections(source,once,desktop
    ?{groupReturn:'return c.mergeLeadingIntoPromotedSection===!0?{leading:[],promoted:[...ie,...ee],others:le}:{leading:ie,promoted:ee,others:le}',
      promotedAnchor:'HP(smt,{models:B.promoted,title:c?.promotedSectionTitle',modelsVar:'B',jsx:'HP',fmt:'smt',renderModel:'x'}
    :{groupReturn:'return l.mergeLeadingIntoPromotedSection===!0?{leading:[],promoted:[...te,...Z],others:ie}:{leading:te,promoted:Z,others:ie}',
      promotedAnchor:'K5(e9t,{models:N.promoted,title:l?.promotedSectionTitle',modelsVar:'N',jsx:'K5',fmt:'e9t',renderModel:'w'});
  ...
  const models=desktop?'c':'l', mapper=desktop?'V=>DLt(V)':'U=>qon(U)';
  ...
  if(source.includes('const __chatgptLocal=__isChatgptBridgeModel(u?.requestedModel?.modelId??i?.modelId);')){
    ...
    const before=desktop?'async run(e,t,n,i,r,s,o,a,c,l,u){const h=ZHf(u,':'async run(t,e,n,i,r,s,o,a,l,c,u){const d=IES(u,';
  ...
  const native=desktop?'Qh(this.storageService,"useDedicatedLocalAgentRuntimeHost")':'jp(this.storageService,"useDedicatedLocalAgentRuntimeHost")';
  source=once(source,native,'(__isClaudeBridgeModel('+(desktop?'g':'p')+')&&Boolean(this.environmentService.remoteAuthority)||'+native+')');
  const activation=desktop?'function Jdp(e){return qc.localMode&&e?.get(Poy,-1)==="true"}':'function aNg(t){return Ml.localMode&&t?.get(gNg,-1)==="true"}';
  ...
  const usage=desktop
    ?{jsx:'HIy',useState:'Xur',useEffect:'Zxy',card:'sb',zs:'Ts',bar:'yA',barStyle:'idr',
      fn:'function KIy(e){const t=Pbp(119)',
      children:'title:"Plan & Usage",children:[ln,qt,un,pn]',
      childrenGpt:'title:"Plan & Usage",children:[ln,qt,un,pn,HIy(__chatgptUsageSection,{})]'}
    :{jsx:'Sik',useState:'pds',useEffect:'yik',card:'Rf',zs:'Os',bar:'Om',barStyle:'CEi',
      fn:'function Cik(t){const e=gBg(119)',
      children:'title:"Plan & Usage",children:[pt,bt,gt,yt]',
      childrenGpt:'title:"Plan & Usage",children:[pt,bt,gt,yt,Sik(__chatgptUsageSection,{})]'};
  ...
  source=patchSubagentBubbles(source,surface,'3.21.9');
  source=patchSubagentLifecycle(source,surface,'claude-subscription/','3.21.9');
...
if(process.argv.includes('--check')){console.log('Cursor 3.21.9 Claude patch candidates passed syntax and anchor checks.');process.exit();}
