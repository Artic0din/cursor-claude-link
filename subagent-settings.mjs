...
// These helpers are copied into the workbench bundle: they must not close over
// Node imports. Prefix is an argument; patchers validate it before injecting.
...
export function configureTaskProps(input, props, prefix) {
  ...
  return {...props,parentModelParameters:input.modelParameters,
    subagentModels:{...props.subagentModels,__subscriptionSelection:resolved?.type==='model'&&selected?
...
export function patchSubagentSettingsWorkbench(source, prefix) {
