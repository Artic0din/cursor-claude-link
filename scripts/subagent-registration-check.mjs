...
export async function verifySubagentRegistration(source) {
 ...
 const factory=new Function('BK','Roe','Xr','Zs','Xe','vt','$Be','O7e','__ensureClaudeTaskBubble','__ensureChatgptTaskBubble','return ({'+method+'})._waitForParentTaskBubbleIfPossible');
 ...
 const run=helper=>factory(trim,trim,{TOOL_FORMER:1},{TOOL_FORMER:1},{TASK_V2:2},{TASK_V2:2},Params,Params,helper,()=>{}).call(service,request);
 ...
 await run(ensureClaudeTaskBubble);assert.equal(bubbles.size,1);
