...
if(version!=='3.20.17'||product.commit!=='0c32194e3fb5ffaced9fb36430b860ec301e1fc0')throw new Error('This patch supports Cursor 3.20.17 build 0c32194 only.');
...
for(const surface of ['desktop','glass']){
  ...
  const models=desktop?'c':'l', mapper=desktop?'V=>ipn(V)':'U=>Izn(U)';
  ...
  if(source.includes('const __chatgptLocal=__isChatgptBridgeModel(u?.requestedModel?.modelId??i?.modelId);')){
    ...
    const before=desktop?'async run(e,t,n,i,r,s,o,a,c,l,u){const h=Bjf(u,':'async run(t,e,n,i,r,s,o,a,l,c,u){const d=LSS(u,';
  ...
  const native=desktop?'Gh(this.storageService,"useDedicatedLocalAgentRuntimeHost")':'Gp(this.storageService,"useDedicatedLocalAgentRuntimeHost")';
  source=once(source,native,'(__isClaudeBridgeModel('+(desktop?'g':'p')+')&&Boolean(this.environmentService.remoteAuthority)||'+native+')');
  const activation=desktop?'function jup(e){return Zc.localMode&&e?.get(vey,-1)==="true"}':'function mAg(t){return wl.localMode&&t?.get(wAg,-1)==="true"}';
  ...
  const usage=desktop
    ?{jsx:'Kby',useState:'Glr',useEffect:'iby',card:'__',zs:'zs',bar:'xA',barStyle:'Qlr',
      fn:'function e_y(e){const t=Mfp(119)',
      children:'title:"Plan & Usage",children:[yn,an,Kt,xn]',
      childrenGpt:'title:"Plan & Usage",children:[yn,an,Kt,xn,Kby(__chatgptUsageSection,{})]'}
    :{jsx:'Ez1',useState:'Sus',useEffect:'Tz1',card:'iv',zs:'Js',bar:'Im',barStyle:'CTi',
      fn:'function Az1(t){const e=o3g(119)',
      children:'title:"Plan & Usage",children:[ft,wt,gt,Tt]',
      childrenGpt:'title:"Plan & Usage",children:[ft,wt,gt,Tt,Ez1(__chatgptUsageSection,{})]'};
