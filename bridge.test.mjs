...
test('live picker-models advertises MAX only when the Cursor patch installed it',async t=>{
  ...
  async function listed(advertiseMaxMode){
    const server=http.createServer(createHandler({config:{...config,advertiseMaxMode},getCatalog:async()=>catalog,inferRequest:async()=>({answer:{text:'',tool_calls:[]}})}));
    server.listen(0,'127.0.0.1');await once(server,'listening');
    t.after(()=>new Promise(resolve=>{server.closeAllConnections();server.close(resolve);}));
    const base='http://127.0.0.1:'+server.address().port;
    return (await (await fetch(base+'/picker-models',{headers})).json()).models[0];
  }
  const gated=await listed(false);
  assert.equal(gated.supportsMaxMode,false);
  assert.equal(gated.contextTokenLimit,200000);
  assert.equal(gated.contextTokenLimitForMaxMode,1000000);
  assert.equal((await listed(true)).supportsMaxMode,true);
});
