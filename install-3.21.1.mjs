...
if(version!=='3.21.1'||product.commit!=='74f717017ddcbf0554cd8c91ec7e2fb56983a070')throw new Error('This patch supports Cursor 3.21.1 build 74f7170 only.');
...
for(const surface of ['desktop','glass']){
  ...
  source=patchPickerSections(source,once,desktop
    ?{groupReturn:'return c.mergeLeadingIntoPromotedSection===!0?{leading:[],promoted:[...re,...ee],others:ce}:{leading:re,promoted:ee,others:ce}',
      promotedAnchor:'HP(rmt,{models:B.promoted,title:c?.promotedSectionTitle',modelsVar:'B',jsx:'HP',fmt:'rmt',renderModel:'x'}
    :{groupReturn:'return l.mergeLeadingIntoPromotedSection===!0?{leading:[],promoted:[...ne,...Z],others:ie}:{leading:ne,promoted:Z,others:ie}',
      promotedAnchor:'z5(t9t,{models:N.promoted,title:l?.promotedSectionTitle',modelsVar:'N',jsx:'z5',fmt:'t9t',renderModel:'w'});
  ...
  const models=desktop?'c':'l', mapper=desktop?'V=>xLt(V)':'U=>Hon(U)';
  ...
  if(source.includes('const __chatgptLocal=__isChatgptBridgeModel(u?.requestedModel?.modelId??i?.modelId);')){
    ...
    const before=desktop?'async run(e,t,n,i,r,s,o,a,c,l,u){const h=lHf(u,':'async run(t,e,n,i,r,s,o,a,l,c,u){const d=gTS(u,';
  ...
  const native=desktop?'Zh(this.storageService,"useDedicatedLocalAgentRuntimeHost")':'Bp(this.storageService,"useDedicatedLocalAgentRuntimeHost")';
  source=once(source,native,'(__isClaudeBridgeModel('+(desktop?'g':'p')+')&&Boolean(this.environmentService.remoteAuthority)||'+native+')');
  const activation=desktop?'function fdp(e){return qc.localMode&&e?.get(Usy,-1)==="true"}':'function sDg(t){return Ml.localMode&&t?.get(pDg,-1)==="true"}';
  ...
  const usage=desktop
    ?{jsx:'Kxy',useState:'Uur',useEffect:'ixy',card:'sb',zs:'Ts',bar:'yA',barStyle:'zur',
      fn:'function eIy(e){const t=Xvp(119)',
      children:'title:"Plan & Usage",children:[ln,qt,un,pn]',
      childrenGpt:'title:"Plan & Usage",children:[ln,qt,un,pn,Kxy(__chatgptUsageSection,{})]'}
    :{jsx:'ank',useState:'Yus',useEffect:'onk',card:'xf',zs:'Os',bar:'Om',barStyle:'dEi',
      fn:'function unk(t){const e=p4g(119)',
      children:'title:"Plan & Usage",children:[pt,bt,gt,yt]',
      childrenGpt:'title:"Plan & Usage",children:[pt,bt,gt,yt,ank(__chatgptUsageSection,{})]'};
  ...
  source=patchSubagentBubbles(source,surface,'3.21.1');
  source=patchSubagentLifecycle(source,surface,'claude-subscription/','3.21.1');
...
if(process.argv.includes('--check')){console.log('Cursor 3.21.1 Claude patch candidates passed syntax and anchor checks.');process.exit();}
