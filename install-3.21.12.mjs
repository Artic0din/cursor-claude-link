...
if(version!=='3.21.12'||product.commit!=='05ddb9e824590e2c1db6bd2548dd71bf67ac9d20')throw new Error('This patch supports Cursor 3.21.12 build 05ddb9e only.');
...
for(const surface of ['desktop','glass']){
  ...
  const models=desktop?'c':'l', mapper=desktop?'V=>DLt(V)':'U=>Von(U)';
  ...
  if(source.includes('const __chatgptLocal=__isChatgptBridgeModel(u?.requestedModel?.modelId??i?.modelId);')){
    ...
    const before=desktop?'async run(e,t,n,i,r,s,o,a,c,l,u){const h=a6d(u,':'async run(t,e,n,i,r,s,o,a,l,c,u){const d=wum(u,';
  ...
  const native=desktop?'Qh(this.storageService,"useDedicatedLocalAgentRuntimeHost")':'Pp(this.storageService,"useDedicatedLocalAgentRuntimeHost")';
  source=once(source,native,'(__isClaudeBridgeModel('+(desktop?'g':'p')+')&&Boolean(this.environmentService.remoteAuthority)||'+native+')');
  const activation=desktop?'function nhp(e){return jc.localMode&&e?.get(Ooy,-1)==="true"}':'function lNg(t){return Al.localMode&&t?.get(fNg,-1)==="true"}';
  ...
  source=patchSubagentBubbles(source,surface,'3.21.12');
  source=patchSubagentLifecycle(source,surface,'claude-subscription/','3.21.12');
...
if(process.argv.includes('--check')){console.log('Cursor 3.21.12 Claude patch candidates passed syntax and anchor checks.');process.exit();}
