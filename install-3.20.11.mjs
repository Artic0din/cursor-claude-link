...
if(version!=='3.20.11'||product.commit!=='69d099d6568dc97e110ba8184614faf51c4040b0')throw new Error('This patch supports Cursor 3.20.11 build 69d099d only.');
...
for(const surface of ['desktop','glass']){
  ...
  if(source.includes('const __chatgptLocal=__isChatgptBridgeModel(u?.requestedModel?.modelId??i?.modelId);')){
    ...
    const before=desktop?'async run(e,t,n,i,r,s,o,a,c,l,u){const h=Wjf(u,':'async run(t,e,n,i,r,s,o,a,l,c,u){const d=EyS(u,';
  ...
  const activation=desktop?'function ndp(e){return Qc.localMode&&e?.get(ity,-1)==="true"}':'function SIg(t){return Cl.localMode&&t?.get(AIg,-1)==="true"}';
  ...
  const usage=desktop
    ?{jsx:'SDm',useState:'OOm',useEffect:'FOm',card:'Jb',zs:'zs',bar:'xA',barStyle:'nur',
      fn:'function W_y(e){const t=Vfp(119)',
      children:'title:"Plan & Usage",children:[yn,an,Kt,xn]',
      childrenGpt:'title:"Plan & Usage",children:[yn,an,Kt,xn,SDm(__chatgptUsageSection,{})]'}
    :{jsx:'cVb',useState:'fgr',useEffect:'ffd',card:'Yf',zs:'Ks',bar:'Am',barStyle:'mTi',
      fn:'function kH1(t){const e=hDg(119)',
      children:'title:"Plan & Usage",children:[ft,wt,gt,Ct]',
      childrenGpt:'title:"Plan & Usage",children:[ft,wt,gt,Ct,cVb(__chatgptUsageSection,{})]'};
...
if(process.argv.includes('--check')){console.log('Cursor 3.20.11 Claude patch candidates passed syntax and anchor checks.');process.exit();}
